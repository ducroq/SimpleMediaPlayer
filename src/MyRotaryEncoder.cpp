...
#else                //fast MCU
  case 0b0100:
  case 0b1011: //CCW states, 1 count  per click
...
{
  bool reading = !digitalRead(_encoderButton); //HIGH not pressed & LOW pressed, because internal pull-up resistor is enabled

  // If the switch changed, due to noise or pressing:
  if (reading != _lastButtonState) // reset the debouncing timer
...
{
  if ((millis() - lastSampleTime) > 0)
  {
    readAB();
    readPushButton();
  }
  lastSampleTime = millis(); // Set variable to current millis() timer
}


int8_t RotaryEncoder::buttonChange()
{
  int8_t ret = (int8_t)_buttonState - (int8_t)_prevButtonState;
...
int16_t RotaryEncoder::positionChange()
{
  int8_t ret = _counter - _prevCounter;
